...
class KnowledgeOnlyModel(nn.Module):
    ...
    def __init__(self, txt_input_dim=768, txt_out_size=300, knowledge_types=[2, 3],
        ...
        # === New: branch-specific heads + gated late fusion ===
        # K is small (len(knowledge_types) == 2 for ANP+Attr), so Linear(K→2) is a good learnable aggregator.
        self.branch_a_head = nn.Sequential(  # atomic branch logits
            nn.LayerNorm(len(self.knowledge_types)),
            nn.Linear(len(self.knowledge_types), 2)
        )
        self.branch_b_head = nn.Sequential(  # compositional branch logits
            nn.LayerNorm(len(self.knowledge_types)),
            nn.Linear(len(self.knowledge_types), 2)
        )
        self.final_fusion = GatedLateFusion(num_classes=2)
    ...
    def forward(self, texts, mask_batch, t1_word_seq, txt_edge_index, gnn_mask, 
        ...
        # Per-branch learnable aggregation to class logits
        logits_a = self.branch_a_head(a1)  # [B, 2]
        logits_b = self.branch_b_head(a2)  # [B, 2]     # [B, 2]

        # Final logits
        predictions = self.final_fusion(logits_a, logits_b)           # [B, 2]
        return predictions
...
class HybridModel(nn.Module):
    ...
    def __init__(self, txt_input_dim=768, txt_out_size=300, img_input_dim=768,
        ...
        # Output layers
        num_classes = getattr(self, "num_classes", 2)  # keep your default
        self.text_image_output = nn.Linear(2 * self.img_patch, num_classes)
        self.knowledge_output = nn.Linear(self.txt_out_size, num_classes)
        self.final_fusion = GatedLateFusion(num_classes=num_classes) 
    ...
    def forward(self, imgs, texts, mask_batch, img_edge_index, t1_word_seq,txt_edge_index, gnn_mask, np_mask, knowledge_inputs, knowledge_masks):
        ...
        # Final fusion
        predictions = self.final_fusion(text_image_pred, knowledge_pred)
